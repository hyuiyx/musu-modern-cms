(() => {
  ...
  function installVideoEditor(){
    ...
    section.innerHTML=`<h2>视频管理 V4.3.3</h2><form id="videoV433Form" class="assetBox videoV433Form"><input name="id" type="hidden"><div class="form"><input name="title" required placeholder="视频标题"><input name="slug" placeholder="URL Slug，可留空"><select name="status"><option value="draft">草稿</option><option value="published">发布</option></select><input name="sort_order" type="number" value="0" placeholder="排序"></div><textarea name="description" placeholder="视频描述"></textarea><h3>视频文件</h3><input name="file" type="file" accept="video/mp4,video/webm,video/ogg,video/quicktime"><p class="helpText">新建必须选择文件，编辑时不选则保留原文件，最大 100 MB。</p><video id="videoV433Preview" controls preload="metadata" class="hide"></video><h3>视频封面</h3><input name="poster" type="file" accept="image/jpeg,image/png,image/webp,image/gif"><label class="removeCheck"><input name="remove_poster" type="checkbox" value="1">移除当前封面</label><img id="videoV433Poster" class="posterPreview hide" alt="视频封面预览"><progress id="videoV433Progress" value="0" max="100"></progress><div id="videoV433Result" class="saveResult"></div><div class="inline"><button type="submit">保存修改</button><button id="videoV433New" type="button" class="secondary">新建</button><button id="videoV433Delete" type="button" class="danger">删除当前视频</button></div></form><h3>现有视频</h3><div id="videoList" class="videoV433List"></div>`;
    const form=q('#videoV433Form'),preview=q('#videoV433Preview'),poster=q('#videoV433Poster'),result=q('#videoV433Result');
    window.loadVideosV433=async()=>{videos=await call('/api/admin/videos');q('#videoList').innerHTML=videos.map(v=>`<div class="row"><b>${safe(v.title)}</b><span>${safe(v.status)}</span><span>${v.file_size?(v.file_size/1048576).toFixed(1)+' MB':''}</span><span class="${v.video_key?'':'missingFile'}">${v.video_key?safe(v.description||''):'R2 文件可能已缺失'}</span><div class="inline"><button type="button" onclick="editVideoV433(${v.id})">编辑</button><button type="button" class="danger" onclick="deleteVideoV433(${v.id})">删除</button></div></div>`).join('')||'<p>暂无视频</p>'};
    window.editVideoV433=id=>{const v=videos.find(x=>Number(x.id)===Number(id));if(!v)return alert('找不到视频记录');form.reset();form.elements.id.value=v.id;form.elements.title.value=v.title||'';form.elements.slug.value=v.slug||'';form.elements.status.value=v.status||'draft';form.elements.sort_order.value=v.sort_order||0;form.elements.description.value=v.description||'';if(v.video_key){preview.src=mediaUrl(v.video_key);preview.classList.remove('hide');preview.load()}else{preview.removeAttribute('src');preview.classList.add('hide')}if(v.poster_key){poster.src=mediaUrl(v.poster_key);poster.classList.remove('hide')}else{poster.removeAttribute('src');poster.classList.add('hide')}result.textContent=`正在编辑视频 ID：${v.id}`;form.scrollIntoView({behavior:'smooth',block:'start'})};
    window.deleteVideoV433=async id=>{const v=videos.find(x=>Number(x.id)===Number(id));if(!v)return;if(!confirm(`确定删除“${v.title}”？将删除 D1 记录并尝试删除 R2 文件。`))return;try{await call(`/api/admin/videos/${id}`,{method:'DELETE'});if(String(form.elements.id.value)===String(id))reset();await window.loadVideosV433();alert('已删除，残留位置已清除')}catch(e){alert('删除失败：'+e.message)}};
    const reset=()=>{form.reset();form.elements.id.value='';form.elements.sort_order.value=0;preview.pause();preview.removeAttribute('src');preview.load();preview.classList.add('hide');poster.removeAttribute('src');poster.classList.add('hide');q('#videoV433Progress').value=0;result.textContent=''};
    q('#videoV433New').onclick=reset;q('#videoV433Delete').onclick=()=>{const id=form.elements.id.value;if(!id)return alert('请先点击列表中的“编辑”');window.deleteVideoV433(id)};
    ...
    form.onsubmit=async e=>{e.preventDefault();const id=form.elements.id.value,file=form.elements.file.files[0];if(!id&&!file)return alert('新建视频必须选择文件');result.textContent='保存中...';try{const d=await upload(id?`/api/admin/videos/${id}`:'/api/admin/videos',new FormData(form),q('#videoV433Progress'));if(!id)form.elements.id.value=d.id;result.textContent='保存成功';await window.loadVideosV433();alert('视频已保存')}catch(err){result.textContent='保存失败：'+err.message;alert(err.message)}};
    document.querySelectorAll('[data-tab="videos"]').forEach(b=>b.addEventListener('click',()=>window.loadVideosV433()));
    window.loadVideosV433().catch(e=>{result.textContent=e.message});
  }
